...
foreach($ADUser in $AADUsers){
    ...
    $ADUserepobj | Add-Member NoteProperty -Name "Created DateTime (UTC)" -Value $ADUser.createdDateTime.Substring(0, $ADUser.createdDateTime.IndexOf('T'))
    if (($ADUser.signInActivity).lastSignInDateTime) {$ADUserepobj | Add-Member NoteProperty -Name "Last interactive Signin (UTC)" -Value ($ADUser.signInActivity).lastSignInDateTime.Substring(0, $ADUser.signInActivity.lastSignInDateTime.IndexOf('T'))}else{$ADUserepobj | Add-Member NoteProperty -Name "Last interactive Signin (UTC)" -Value ""}
    if (($ADUser.signInActivity).LastNonInteractiveSignInDateTime) {$ADUserepobj | Add-Member NoteProperty -Name "Last Success Non-interactive Signin (UTC)" -Value ($ADUser.signInActivity).LastNonInteractiveSignInDateTime.Substring(0, $ADUser.signInActivity.LastNonInteractiveSignInDateTime.IndexOf('T'))}else{$ADUserepobj | Add-Member NoteProperty -Name "Last Success Non-interactive Signin (UTC)" -Value ""}
